...
def config():
    ...
    if request.method == 'POST':
        # フォームデータからYAML設定を更新
        config_data = {
            'elapsed_years': int(request.form.get('elapsed_years', 15)),
            'purchase_price': request.form.get('purchase_price', '420,000,000'),
            'initial_capital_ratio': float(request.form.get('initial_capital_ratio', 0.25)),
            'loan': {
                'annual_rate': float(request.form.get('loan_annual_rate', 0.035)),
                'years': int(request.form.get('loan_years', 35)),
                'start_month': int(request.form.get('loan_start_month', 4))
            },
            'building': {
                'cost': request.form.get('building_cost', '35,654,400'),
                'statutory_life': int(request.form.get('building_statutory_life', 34))
            },
            'equipment': {
                'cost': request.form.get('equipment_cost', '0'),
                'statutory_life': int(request.form.get('equipment_statutory_life', 15))
            },
            'tax': {
                'land_assessed_value': request.form.get('tax_land_assessed_value', '88,559,000'),
                'building_assessed_value': request.form.get('tax_building_assessed_value', '35,654,400'),
                'land_area_m2': float(request.form.get('tax_land_area_m2', 273.76)),
                'units': int(request.form.get('tax_units', 12))
            }
        }
